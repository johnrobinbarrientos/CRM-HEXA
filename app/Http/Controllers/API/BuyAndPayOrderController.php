...
class BuyAndPayOrderController extends Controller
{
    ...
    {
        $orders = BuyAndPayOrder::whereNull('deleted_at')->with('Supplier')->with('ItemGroup')->with('OrderReasonCode')->with('ItemAssetGroup')->get();
        return response()->json(['success' => 1, 'rows' => $orders], 200);
    }

    public function saveOrder()
    {
        $orders = request()->uuid ? BuyAndPayOrder::find(request()->uuid) : new BuyAndPayOrder();
        $auth = \Auth::user();
        ...
        $orders = BuyAndPayOrder::find($orders->uuid);
    ...
    {
        $order = BuyAndPayOrder::find($order_uuid);

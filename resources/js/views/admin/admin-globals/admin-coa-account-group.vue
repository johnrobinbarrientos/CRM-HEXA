        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="mb-3 card-subtitle">
                    Below are the list of active Account Group.
                </div>
                ...
                <div class="table-responsive">
                    <table class="table mb-0 table table-striped">
                        ...
                        <tbody>
                            <tr v-for="(account, index) in accountGroups" :key="account.uuid">
                                <td>
                                    <a href="javascript:void(0)"  @click="OPEN_MODAL('#modalAccountGroup');setData(account)" class="btn btn-sm btn-light"><i class="bx bx-pencil"></i></a>
    ...
    methods: {
        ...
        getAccountGroups: function () {
           var scope = this
            ...
            scope.accountGroups = []
            scope.GET('admin/coa-account-group?keyword=' + scope.searchKeyword + '&page=' + scope.listCurrentPage + '&take=' + scope.listItemPerPage).then(res => {
        ...
        save: function () {
            ...
            scope.POST('admin/coa-account-group', scope.formdata).then(res => {
                if (res.success) {
                    window.swal.fire({
                        position: 'center',
                        icon: 'success',
        ...
        remove: function (data) {
            ...
            window.swal.fire({
                ...
                if (result.value) {
                    scope.POST('admin/coa-account-group/delete', data).then(res => {
                        if (res.success) {
                            window.swal.fire({
                                position: 'center',
                                icon: 'success',

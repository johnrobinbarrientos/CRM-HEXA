    ...
    <div>
        <div style="margin-bottom:40px;" class="nk-fmg-body-head d-none d-lg-flex">
            <div class="nk-fmg-search">
                ...
                <input type="text" class="form-control border-transparent form-focus-none" placeholder="Search Supplier Group">
            </div>
            <div class="nk-fmg-actions">
                <ul class="nk-block-tools g-3">
                    <li>
                        <a href="javascript:void(0)" @click="OPEN_MODAL('#modalSupplierGroup');resetData()" class="btn btn-primary" data-toggle="modal">
                            <em class="icon ni ni-plus"></em> <span>New Supplier Group</span>
                        </a>
                    </li>
                </ul>
            </div>
                                ...
                                <table class="table table-tranx">
                                    ...
                                    <tbody>
                                        <tr v-for="(group, index) in groups" :key="group.uuid" class="tb-tnx-item">
                                            ...
                                            <td>
                                                <span class="">
                                                    <a href="javascript:void(0)"  @click="OPEN_MODAL('#modalSupplierGroup');setData(group)" class="btn btn-sm btn-light"><em class="icon ni ni-pen2"></em></a>
...
export default {
    ...
    methods: {
        ...
        save: function () {
            var scope = this
            scope.POST('suppliers/supplier-group/save', scope.formdata).then(res => {
                if (res.success) {
                    window.swal.fire({
                        position: 'center',
                        icon: 'success',
        ...
        update: function () {
            ...
            window.swal.fire({
                ...
                if (result.value) {
                    scope.PUT('suppliers/supplier-group/update', scope.formdata).then(res => {
                        if (res.success) {
                            window.swal.fire({
                                position: 'center',
                                icon: 'success',

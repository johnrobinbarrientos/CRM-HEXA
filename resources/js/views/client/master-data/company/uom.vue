<template>
    <div>
        <div class="actions-bar">
            <div class="w-100">
                <h1 class="title"><i class="las la-list-ul"></i>Global UOM</h1>
            </div>
            <div class="bar-right">
                ...
                <a href="javascript:void(0)" @click="OPEN_MODAL('#modalUom');resetData()" class="hx-btn hx-btn-shineblue" data-toggle="modal">
            ...
            <div class="col-lg-6">
                <div class="table-responsive">
                    <table class="table table-striped table-hover table-bordered">
                        ...
                        <tbody>
                            <tr v-for="(uom) in Uoms" :key="uom.uuid" class="tb-tnx-item">
                                <td width="100">
                                    <span class="hx-table-actions">
                                        <b-dropdown split text="Edit" size ="sm" class="m-2" href="javascript:void(0)" @click="OPEN_MODAL('#modalUom');setData(uom)">
                                            <b-dropdown-item href="javascript:void(0)" @click="OPEN_MODAL('#modalUom');setData(uom)">Edit</b-dropdown-item>
                                            <b-dropdown-item href="javascript:void(0)" @click="remove(uom)">Delete</b-dropdown-item>
                                        </b-dropdown>
                                    </span>
                                </td>
                                <td>{{ uom.uom }}</td>
...
export default {
    ...
    methods: {
        getUoms: function () {
            ...
            scope.Uoms = []
            scope.GET('globals/uom?keyword=' + scope.searchKeyword + '&page=' + scope.listCurrentPage + '&take=' + scope.listItemPerPage).then(res => {
                scope.Uoms = res.rows
        ...
        save: function () {
            var scope = this
            scope.POST('globals/uom', scope.formdata).then(res => {
                if (res.success) {
                    window.swal.fire({
                        position: 'center',
                        icon: 'success',
        ...
        remove: function (data) {
            ...
            window.swal.fire({
                ...
                if (result.value) {
                    scope.POST('globals/uom/delete', data).then(res => {
                        if (res.success) {
                            window.swal.fire({
                                position: 'center',
                                icon: 'success',

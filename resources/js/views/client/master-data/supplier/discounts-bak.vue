                                    ...
                                    <tr v-if="group.edit == true" :key="'group-items-' + index">
                                        <td style="padding:0px;"  colspan="3">
                                            <div style="padding:5px; background:#f5f5f5; border: 2px solid #ccc;">
                                                ...
                                                <input v-model="group.name" class="form-control-gray-medium"  v-bind:class="{'error' : group.name_error}" type="text" placeholder="Enter group name">
                                                ...
                                                <div style="padding:8px; padding-bottom:15px; border:1px solid #ccc; background:#fff; margin-top:20px;">
                                                    ...
                                                    <table style="margin-top:5px;" class="table table-bordered table-hover">
                                                        ...
                                                        <tbody>
                                                            <tr style="background:#fff;" v-for="(discount,discount_index) in group.discounts" :key="'edit-group-discount-' + discount_index ">
    ...
    methods: {
        ...
        getDiscounts: function () {
            ...
            scope.GET('suppliers/' + supplier_discount_group_uuid + '/supplier-base-discount-group-details').then(res => {
                scope.discounts = (res.rows) ? res.rows : []
            })
        },
        ...
        editGroup: function (data) {
           ...
           scope.$set(scope.selected_group,'copy', copy)
        },
        ...
        cancelGroup: function () {
            ...
            if (!scope.selected_group.new) {
                scope.selected_group = JSON.parse(JSON.stringify(scope.selected_group.copy)) 
            }
           
            scope.selected_group.edit = false     
            scope.selected_group.view = false    
            CLOSE_MODAL('#modalDiscounts'); 
        },

<template>
    <div>
        <div class="actions-bar">
            <div class="w-100">
                <h1 class="title"><i class="las la-list-ul"></i>Item Type</h1>
            </div>
            <div class="bar-right">
                ...
                <a href="javascript:void(0)" @click="OPEN_MODAL('#modalItemType');resetData()" class="hx-btn hx-btn-shineblue" data-toggle="modal">
...
export default {
    ...
    methods: {
        getCatItemType: function () {
            ...
            scope.categories = []
            scope.GET('items/cat-item-type?keyword=' + scope.searchKeyword + '&page=' + scope.listCurrentPage + '&take=' + scope.listItemPerPage).then(res => {
        ...
        resetData: function () {
            ...
            scope.formdata.item_type = ''
        },
        setData: function (data) {
            var scope = this
            scope.formdata.uuid = data.uuid
            scope.formdata.item_type = data.item_type
        },
        save: function () {
            var scope = this
            scope.POST('items/cat-item-type', scope.formdata).then(res => {
                if (res.success) {
                    window.swal.fire({
                        position: 'center',
                        icon: 'success',
                        ...
                        scope.CLOSE_MODAL('#modalItemType')
                    })
                } else {
        ...
        remove: function (data) {
            ...
            window.swal.fire({
                ...
                if (result.value) {
                    scope.POST('items/cat-item-type/delete', data).then(res => {
                        if (res.success) {
                            window.swal.fire({
                                position: 'center',
                                icon: 'success',

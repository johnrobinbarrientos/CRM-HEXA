<template>
    <div>
        ...
        <div class="actions-bar">
            <div class="w-100">
                <h1 class="title"><i class="las la-list-ul"></i>Customer Type</h1>
            </div>
            <div class="bar-right">
...
export default {
    ...
    methods: {
        getCustomerTypes: function () {
            ...
            scope.customerTypes = []
            scope.GET('customers/customer-type?keyword=' + scope.searchKeyword + '&page=' + scope.listCurrentPage + '&take=' + scope.listItemPerPage).then(res => {
        ...
        save: function () {
            var scope = this
            scope.POST('customers/customer-type', scope.formdata).then(res => {
            if (res.success) {
                window.swal.fire({
                    position: 'center',
                    icon: 'success',
        ...
        remove: function (data) {
            ...
            window.swal.fire({
                ...
                if (result.value) {
                    scope.POST('customers/customer-type/delete', data).then(res => {
                        if (res.success) {
                            window.swal.fire({
                                position: 'center',
                                icon: 'success',
